...
def creat_bubble_representation(image, text_polys, min_area, max_area, min_circularity, min_solidity):
    polygons = list(map(lambda box: np.array(box, dtype=np.int32).reshape(-1, 1, 2), text_polys))
    ...
    good_size_contours = [cnt for cnt in contours if min_area < cv2.contourArea(cnt) < max_area]
    ...
    bubble_representation = [
        {
            ...
            "is_bubble": cnt_circularity > min_circularity and cnt_solidity > min_solidity,
...
image_brep = creat_bubble_representation(image, text_polys, 500, 50000, 0.2, 0.2)
