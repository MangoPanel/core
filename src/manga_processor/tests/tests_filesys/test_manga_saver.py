...
class TestMangaLoader(TestCase):
    ...
    def _make_dummy_files(self, filenames: list[str]):
        ...
        if len(png_files) < 3:
            raise FileNotFoundError(f"Not enough PNG files in test data directory. Found {len(png_files)}, need at least 3.")
    ...
    def test_save_manga_pdf(self):
        ...
        page_paths = [MangaPagePath(index=i, path=Path(path)) for i, path in enumerate(paths)]
        dummy_manga = self._make_dummy_manga(paths=page_paths)

        save_path= self.temp_input_dir.parent / f"{self.temp_input_dir.name}_saved"/ "manga.pdf"

        self.manga_saver.save_to_pdf(manga=dummy_manga, output_path=save_path)
 

        self.assertTrue(save_path.is_file(), "the pdf file is missing or did not get saved")
        self.assertEqual(save_path.suffix, ".pdf", "the saved file should be in pdf format")
        saved_files = list(save_path.parent.glob("*"))
        self.assertEqual(len(saved_files), 1, "Only 1 file should exist in the save directory")

        doc = pymupdf.open(save_path)
        page_count = doc.page_count
        self.assertEqual(page_count, 3, f"Document should have 3 pages while it has {page_count}")
